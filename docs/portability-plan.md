...
- [x] **0.5 Minimal testability seams (behavior-preserving).** Introduce injectable deps ONLY where 0.3/0.4 need them, with the existing concrete impl as the default so runtime behavior is identical:
...
- [x] **0.8 Hyprland-specific lifecycle regression tests** (no-regression for the existing appliance; stays a Hyprland-only extra, not portable core). Drive `reconcile`/`apply` against a fake session-count + marker and a stub launcher/killer:
...
- [x] **1.1 Define Seam-1 interface.** New `internal/osproc` with `type Source interface { Enumerate() ([]Info, error); Watch(ctx, pid, onDeath) error; Stop(pid) }`. Move existing `/proc`+`pidfd` code behind `osproc_linux.go`. `discovery` and `procwatch` collapse into / call this. DoD: daemon builds with `osproc` injected; existing tests pass; `go vet` clean. *(Done: `Source` adds `Read`; `procwatch` absorbed into `source_linux.go` + `source_darwin.go` stub turns darwin build green; ⚠ #11 POLLERR fix landed. `discovery` keeps its proc-backed scan loop — repointing it through `osproc.Enumerate` is deferred to Phase 4 with the macOS source.)*
- [x] **1.2 Define Seam-2 interface.** New `internal/terminal` with `type Locator interface { Locate(ctx, tty string) (*PaneRef, error); Activate(ctx, PaneRef) error; Name() string }`. Move wezterm behind `terminal/wezterm.go`. Add `terminal/none.go`. DoD: mapping calls the interface; wezterm path unchanged. *(Done: cwd decoding moved into the seam, flipping ⚠ #8.)*
- [x] **1.3 Define Seam-3 interface.** New `internal/wm` with `type Manager interface { Clients(ctx) ([]Window, error); ActiveWindow(ctx) (Ref, error); Focus(ctx, Ref) error; Subscribe(ctx) (<-chan Event, error); Name() string }`. `Ref` is opaque (interface or tagged string). Move Hyprland behind `wm/hyprland.go`; add `wm/none.go`. Translate Hyprland's raw socket2 events into the neutral `Event` enum here. DoD: daemon's `runHyprlandLoop`/`reconcileOnce` consume neutral events; Hyprland behavior identical. *(Done: `Ref` is the opaque address string; the seam owns 0x-normalization, flipping ⚠ #1.)*
- [x] **1.4 Runtime detection + capability reporting.** New `internal/detect`: probe env and select `{osproc, terminal, wm}` backends. Daemon logs the chosen stack at startup and writes a `capabilities` block into `state.json` (`{observe:true, navigate:bool, wm:"hyprland", terminal:"wezterm"}`). DoD: on dev box, detection picks Hyprland+wezterm; forcing `none` backends via flags yields Observe-only with `navigate:false`.
- [x] **1.5 Graceful Navigate degradation.** `rpc.focus` returns a clean "navigate unsupported on this stack" error instead of "no hyprland address". DoD: with `wm=none`, `focus` returns the typed error; `list`/`subscribe`/hooks unaffected. *(Done: also added `pid:`/`idx:` selectors flipping ⚠ #3, and the ⚠ #7 doc fix.)*
...
- [x] **3.1 tmux backend.** `terminal/tmux.go`: `tmux list-panes -a -F …`; match by `pane_tty`; focus via `select-window`+`select-pane`. Note the tmux↔WM bridge: the WM window owning tmux is the tmux **client's** terminal — Navigate may need both the tmux focus *and* the WM focus of the client's terminal window. DoD: in a tmux session inside any supported terminal, chips resolve; focus selects the right pane. *(Done: a claude in tmux is observed via osproc regardless; the tmux locator adds Navigate by re-locating the pane on the tty. Pure parser unit-tested; `Locate` and `select-window`/`select-pane` **verified live against a real tmux 3.5a server** (Locate found the pane by tty; Activate switched the active window). `Available()` is a socket-existence check, so it never spawns tmux when no server runs.)*
- [ ] **3.2 kitty backend (optional/stretch).** `kitten @ ls` if `$KITTY_LISTEN_ON` set. DoD: deferred unless trivial. *(Deferred per plan.)*
- [x] **3.3 Locator chaining.** Support "tmux pane lives inside a wezterm/foot window" by composing locators (tty→tmux pane, then tmux client tty→WM window). DoD: nested case documented; at least the tmux-in-X11 path works. *(Done in part: `terminal.Chain` tries locators innermost-first (tmux→wezterm) and routes `Activate` by `PaneRef.Backend`; detection builds the chain. `rpc.focus` is now backend-agnostic — it re-locates by tty and raises the WM window best-effort, so no terminal-specific fields are persisted. **Remaining (deferred):** the full tmux→client-tty→WM-window raise — tmux panes currently leave `Mux`=0 so they get tmux-pane focus but no WM raise; documented in `terminal/tmux.go`.)*
