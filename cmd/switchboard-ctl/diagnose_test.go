...
func TestBuildObserverDiagnosticsContentFreeStatesAndShadowMismatch(t *testing.T) {
	...
	snap := state.Snapshot{SchemaVersion: state.CurrentSchemaVersion, Sessions: []state.Session{
		{
			...
			DisplayName: &state.DisplayName{Value: "private-display-name", Origin: state.DisplayNameGenerated, ConversationID: "codex-root"},
	...
	if got[0].ThreadID != "codex-root" || got[0].DisplayNameOrigin != string(state.DisplayNameGenerated) || got[0].ObserverConnection != "not_reported" {
		t.Fatalf("Codex diagnostic = %+v", got[0])
	}
	if got[0].BindingSource != "hook" {
		t.Fatalf("hook binding not reported: %+v", got[0])
	}
	...
	for _, secret := range []string{"/secret/project", "private root", "private prompt", "private-display-name"} {
