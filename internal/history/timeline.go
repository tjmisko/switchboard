...
func BuildSwimlanes(events []Event, end time.Time) []Swimlane {
	...
	for _, ev := range evs {
		...
		//
		// legacyEventMemorySample joins them: the memory-sampling feature was
		// retired 2026-08-26 (docs/seed-replay-memory-plan.md), but day-files
		// recorded before the retirement — or not yet scrubbed — still carry its
		// lines, and they were never lane evidence. The sample fired
		// unconditionally every tick, and a hung, idle, or Ctrl-Z'd process still
