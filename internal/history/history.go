...
const (
	...
	// v3 recorded a per-session memory_sample each reconcile tick. The feature
	// was retired 2026-08-26 — it wrote ~12x the line volume of everything else
	// combined and fattened the store until seeding replay OOM-killed the daemon
	// (docs/seed-replay-memory-plan.md). The type string survives only so
	// readers keep IGNORING the lines old stores still carry.
	legacyEventMemorySample = "memory_sample"
