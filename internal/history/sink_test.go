...
func TestLoadConfigOmittedRetentionKeepsDefaults(t *testing.T) {
	...
	if cfg.MaxBytes != defaultMaxBytes {
		t.Errorf("omitted max_bytes = %d, want default %d", cfg.MaxBytes, int64(defaultMaxBytes))
	}
}
...
func TestLoadConfigMalformedJSONFallsBackToDisabledDefaults(t *testing.T) {
	...
	if cfg.Detail != DetailMinimal || cfg.RetainDays != 90 || cfg.MaxBytes != defaultMaxBytes {
...
func TestExplicitMaxBytesWinsOverTheDefault(t *testing.T) {
	cases := []struct {
		name string
		body string
		want int64
	}{
		{"explicit cap", `{"enabled":true,"max_bytes":500}`, 500},
		// 0 is a deliberate value (unlimited), not an omission, so it must survive
		// rather than be backfilled by the default.
		{"explicit unlimited", `{"enabled":true,"max_bytes":0}`, 0},
		// A config written for the retired memory sampler still parses; its
		// `memory` key is simply ignored and the cap follows the normal rules.
		{"legacy memory key ignored", `{"enabled":true,"memory":true,"max_bytes":500}`, 500},
		{"legacy memory key with omitted cap", `{"enabled":true,"memory":true}`, defaultMaxBytes},
	}
